...
<c:forEach items="${replays }" var="c" varStatus="i">
	<li class="am-g am-list-item-desced am-list-item-thumbed am-list-item-thumb-left am_list_li">
        <div class=" am-u-sm-9 am-list-main am_list_main">
            <a href="javascript:;" class="">回复</a>&nbsp;
            <span style="font-size:10px;">第${i.index+1 }条</span>
            <a style="font-size:10px;" onclick="delreply(${c.id },this);" href="javascript:;">删除</a>
        <div class="am_list_author"><a href="javascript:void(0)"><span class="am_list_author_ico" style="background-image: url(upload/default.png);"></span><span class="name">${c.name }</span></a><span class="am_news_time">&nbsp;•&nbsp;<time class="timeago" >${c.data }</time></span></div>
          <div class="am-list-item-text am_list_item_text ">${c.content }</div>
        </div>
     </li>
     </c:forEach>
     <li class="am-g am-list-item-desced am-list-item-thumbed am-list-item-thumb-left am_list_li">
         ...
         <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">发表评论:</label>
			<div class="formControls col-xs-8 col-sm-9">
			<input type="hidden" value="${username }" name="name" id="name">
			<input type="hidden" value="<%=id %>" name="idd" id="idd">
			<textarea id="editor" name="editor" class="textarea" placeholder="500个字符以内" onKeyUp="$.Huitextarealength(this,100)"></textarea>
